...
async def create_test_db(event_loop: None) -> AsyncGenerator[None]:
...
@pytest.fixture
def fake_uow_with_data(tasks_orm, users_orm) -> Generator[FakeUnitOfWork]:
    """Returns the test UnitOfWork with tasks."""
    _fake_uow = FakeUnitOfWork(tasks_orm, users_orm)
    yield _fake_uow


@pytest.fixture
def fake_uow_with_users(users_orm) -> Generator[FakeUnitOfWork]:
    """Returns the test UnitOfWork with tasks."""
    _fake_uow = FakeUnitOfWork(users=users_orm)
    yield _fake_uow
